...
describe("computeBankScope", () => {
	...
	describe("project label case folding", () => {
		...
		it("folds a mixed-case checkout root to a lowercase tag", () => {
			...
			// Fork identity keys a remote-less repo as `local/<name>/<commonDir hash>`,
			// so assert the folded prefix rather than a bare basename.
			expect(scope.retainTags?.[0]).toStartWith("project:local/casedrepo/");
			expect(scope.recallTags).toEqual(scope.retainTags);
			expect(scope.retainTags?.[0]).not.toContain("CasedRepo");
		});

		it("folds the label a linked worktree inherits from a mixed-case primary root", () => {
			const fromPrimary = computeBankScope(baseConfig({ scoping: "per-project-tagged" }), primaryRoot);
			const fromWorktree = computeBankScope(baseConfig({ scoping: "per-project-tagged" }), worktreeRoot);
			expect(fromWorktree.retainTags?.[0]).toStartWith("project:local/casedrepo/");
			expect(fromWorktree.retainTags).toEqual(fromPrimary.retainTags);
		});
	});
});
