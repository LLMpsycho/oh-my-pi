...
describe("expandInternalUrls", () => {
	...
	it("passes caller cwd and settings to the router when expanding memory URLs", async () => {
		const cwd = "/tmp/session-b";
		const sourcePath = "/tmp/session-b-memory/memory_summary.md";
		const settings = {
			get: (key: string) => (key === "memory.projectKey" ? "github.com/example/project" : undefined),
