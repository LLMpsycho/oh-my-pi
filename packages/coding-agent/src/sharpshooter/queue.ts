...
export async function appendSharpshooterDelta(
	agentDir: string,
	cwd: string,
	delta: SharpshooterDelta,
	projectKey?: string | null,
): Promise<void> {
	const dir = path.join(sharpshooterQueueDir(agentDir, cwd, projectKey), sanitizeSessionId(delta.sessionId));
 ...
 * the queue.
 */
export async function listSharpshooterDeltas(
	agentDir: string,
	cwd: string,
	projectKey?: string | null,
): Promise<SharpshooterSessionDeltas[]> {
	const queueRoot = sharpshooterQueueDir(agentDir, cwd, projectKey);
...
export async function sharpshooterQueueDepth(
	agentDir: string,
	cwd: string,
	projectKey?: string | null,
): Promise<number> {
	const groups = await listSharpshooterDeltas(agentDir, cwd, projectKey);
