...
export async function runSharpshooterConsolidation(options: {
	...
	try {
		return await withFileLock(
			sharpshooterLockPath(options.agentDir, options.cwd, projectKey),
			async () => {
				acquired = true;
...
async function consolidateLocked(
	...
	const projectKey = options.settings.get("memory.projectKey");
	const state = await readSharpshooterState(options.agentDir, options.cwd, projectKey);
	try {
		...
		const groups = await listSharpshooterDeltas(options.agentDir, options.cwd, projectKey);
		if (groups.length === 0) {
			await writeSharpshooterState(
				options.agentDir,
				options.cwd,
				{
					...state,
		...
		const currentFiles = await readCurrentMemoryFiles(options.agentDir, options.cwd, projectKey);
		...
		await recordConsolidationError(options.agentDir, options.cwd, state, message, projectKey);
		return { ran: false, reason: "error", error: message };
	}
}

async function readCurrentMemoryFiles(
	agentDir: string,
	cwd: string,
	projectKey?: string | null,
): Promise<Record<SharpshooterMemoryFile, string>> {
	...
	await Promise.all(
		SHARPSHOOTER_MEMORY_FILES.map(async name => {
			files[name] = await Bun.file(sharpshooterMemoryFilePath(agentDir, cwd, name, projectKey))
				.text()
				.catch(() => "");
		}),
	);
